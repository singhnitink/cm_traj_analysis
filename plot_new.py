...
#print()
#print(aggregated_df.iloc[:,0].to_list())
#print(resnumber_array_initial,contacts_array_initial,counts_array_initial)
...
plt.xticks(np.arange(resnumber_array_initial.min(),resnumber_array_initial.max()+1,(resnumber_array_initial.max()+1-resnumber_array_initial.min())//10), fontsize=18)
plt.yticks(np.arange(resnumber_array_initial.min(),resnumber_array_initial.max()+1,(resnumber_array_initial.max()+1-resnumber_array_initial.min())//10), fontsize=18)
plt.xlim(resnumber_array_initial.min(),resnumber_array_initial.max())
plt.ylim(resnumber_array_initial.min(),resnumber_array_initial.max())
...
cbar.set_label('Number of contacts', fontsize=20, fontweight="bold")
cbar.ax.tick_params(labelsize=20)
plt.xticks(np.arange(resnumber_array.min(),resnumber_array.max()+1,(resnumber_array.max()+1-resnumber_array.min())//10), fontsize=18)
plt.yticks(np.arange(resnumber_array.min(),resnumber_array.max()+1,(resnumber_array.max()+1-resnumber_array.min())//10), fontsize=18)
plt.xlim(resnumber_array.min(),resnumber_array.max())
plt.ylim(resnumber_array.min(),resnumber_array.max())
plt.gca().invert_yaxis()
plt.xlabel("Residue Number", fontsize=20, fontweight="bold")
plt.ylabel("Residue Number", fontsize=20, fontweight="bold")
